...
const TextEditor = ({ 
  ...
  const handleSelection = useCallback(() => {
    ...
    if (start !== end && start >= 0 && end >= 0) {
      ...
      // Only update if selection is meaningful (not just whitespace)
  ...
  const handleChange = useCallback((e) => {
    ...
    // Check for selection after text change with a small delay
    setTimeout(handleSelection, 10);
  }, [onTextChange, handleSelection]);

  // Handle mouse up and key up for selection
